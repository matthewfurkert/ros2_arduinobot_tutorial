...
ArduinobotInterface::~ArduinobotInterface()
{
    if (arduino_.IsOpen()) {
        try {
            ...
            RCLCPP_FATAL_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Something went wrong whilst closing the connection with port " << port_);
        }
    }
}
...
CallbackReturn ArduinobotInterface::on_init(const hardware_interface::HardwareInfo &hardware_info)
{
    ...
    {
        RCLCPP_FATAL(rclcpp::get_logger("Arduinobot Interface"), "No serial port provided, Aborting");
...
CallbackReturn ArduinobotInterface::on_activate(const rclcpp_lifecycle::State &previous_state) 
{
    ...
    {
        RCLCPP_FATAL_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Something went wrong whilst interacting with the port " << port_);
        return CallbackReturn::FAILURE;
    }
...
CallbackReturn ArduinobotInterface::on_deactivate(const rclcpp_lifecycle::State &previous_state)
{
    ...
    if (arduino_.IsOpen())
    {
        ...
        {
            RCLCPP_FATAL_STREAM(rclcpp::get_logger("ArduinobotInterface"), "Something went wrong whilst closing connection with the port " << port_);
            return CallbackReturn::FAILURE;
        }
